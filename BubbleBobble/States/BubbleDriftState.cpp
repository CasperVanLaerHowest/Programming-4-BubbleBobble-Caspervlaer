...
std::unique_ptr<BaseState> BubbleDriftState::Update(float)
{
	auto* bubble = m_pOwner->GetComponent<BubbleStateComponent>();
	if (!bubble)
		return nullptr;
...
void BubbleDriftState::PrepareMovement()
{
	auto* bubble = m_pOwner->GetComponent<BubbleStateComponent>();
	auto* physics = m_pOwner->GetComponent<PhysicsComponent>();
	if (!bubble || !physics)
...
void BubbleDriftState::PushSideways(float direction)
{
	auto* bubble = m_pOwner->GetComponent<BubbleStateComponent>();
	auto* physics = m_pOwner->GetComponent<PhysicsComponent>();
	if (!bubble || !physics)
